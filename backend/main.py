...
async def login_for_access_token(form_data: schemas.LoginRequest, db: Session = Depends(get_db)):
    try:
        ...
        return {"access_token": access_token, "token_type": "bearer", "role": role_name, "username": user.username, "room_number": user.room_number}
...
def get_queue(room: Optional[str] = None, db: Session = Depends(get_db)):
    query = db.query(models.Queue).filter(models.Queue.status == "waiting")
    if room:
        query = query.filter(models.Queue.target_room == room)
    return query.join(models.PriorityLevel).order_by(models.PriorityLevel.weight.asc(), models.Queue.created_at.asc()).all()
...
def get_history(status: Optional[str] = None, limit: int = 50, db: Session = Depends(get_db)):
    ...
    return query.order_by(models.Queue.created_at.desc()).limit(limit).all()
...
@app.get("/panel-links")
def get_panel_links(current_user: models.User = Depends(get_current_active_user)):
    base_url = "http://localhost:5173" # Assuming Vite
    return [
...
class QueueAdmin(ModelView, model=models.Queue):
    column_list = [models.Queue.token_number, models.Queue.patient_name, models.Queue.priority, models.Queue.status, models.Queue.target_dept, models.Queue.doctor, models.Queue.room_number]
